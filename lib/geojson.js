...
function voieToFeature(voie) {
  ...
  const properties = omit(voie, 'numeros')
...
function toponymeToFeature(toponyme) {
  ...
  const properties = omit(toponyme, 'position')
  properties.color = randomColor({luminosity: 'dark', seed: toponyme.codeVoie})
...
function numeroToFeature(numero) {
  ...
  const properties = omit(numero, 'positions')
  properties.type = position.type
...
export function communeVoiesToGeoJson(commune) {
  const toponymes = commune.voies.filter(voie => voie.position)
  const voieWithNumeros = commune.voies.filter(voie => voie.numeros)
...
export function communeNumerosToGeoJson(commune) {
  ...
  for (const voie of commune.voies) {
    if (voie.numeros) {
      const numerosWithPos = voie.numeros.filter(hasPosition)
