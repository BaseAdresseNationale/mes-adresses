...
export function setIfPublished(baseLocale, publishedArray) {
...
export async function searchBAL(codeCommune, userEmail) {
  const userBALs = await request(`/bases-locales/search?codeCommune=${codeCommune}&userEmail=${userEmail}`)
  const publishedIds = await getPublishedIds()

  for (const userBAL of userBALs) {
    setIfPublished(userBAL, publishedIds)
  }
