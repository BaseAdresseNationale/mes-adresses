...
export function getVoies(balId, isDeleted = undefined) {
  let url = `/bases-locales/${balId}/voies`
...
export function getToponymes(balId, isDeleted = undefined) {
  let url = `/bases-locales/${balId}/toponymes`
  if (isDeleted === true || isDeleted === false) {
    url += `?isdeleted=${isDeleted}`
  }
...
export function getNumerosByBal(idBal, token, isDeleted = undefined) {
  const headers = token ? {
    authorization: `Token ${token}`
  } : {}
  let url = `/bases-locales/${idBal}/numeros`
  if (isDeleted === true || isDeleted === false) {
    url += `?isdeleted=${isDeleted}`
  }
...
export function getNumeros(idVoie, token, isDeleted = undefined) {
  const headers = token ? {
    authorization: `Token ${token}`
  } : {}
  let url = `/voies/${idVoie}/numeros`
  if (isDeleted === true || isDeleted === false) {
    url += `?isdeleted=${isDeleted}`
  }
