...
export async function listBasesLocales(fetchDeleted) {
  const allBAL = await request('/bases-locales')

  return fetchDeleted ? allBAL : allBAL.filter(bal => !bal._deleted)
}
...
export async function searchBAL(codeCommune, userEmail, fetchDeleted) {
