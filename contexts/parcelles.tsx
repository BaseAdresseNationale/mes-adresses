...
import {SOURCE as CADASTRE_SOURCE, SOURCE_LAYER as CADASTRE_SOURCE_LAYER, LAYER as CADASTRE_LAYER} from '@/components/map/layers/cadastre'
...
interface ParcellesContextType {
  ...
  handleHoveredParcelle: (value: {id: string, featureId?: string} | null) => void;
...
function getHoveredFeatureId(map: MaplibreMap, id: string): string | undefined {
  const [feature] = map.querySourceFeatures(CADASTRE_SOURCE, {
    sourceLayer: CADASTRE_SOURCE_LAYER.PARCELLES, filter: ['==', ['get', 'id'], id]
  })
...
export function ParcellesContextProvider(props: ChildrenProps) {
  ...
  const [hoveredParcelle, setHoveredParcelle] = useState<{id: string, featureId?: string} | null>(null)
  const [isParcelleSelectionEnabled, setIsParcelleSelectionEnabled] = useState<boolean>(false)
  const [selectedParcelles, setSelectedParcelles] = useState<string[]>([])

  const prevHoveredParcelle = useRef<string | undefined>()

  const handleHoveredParcelle = useCallback((hovered: {id: string, featureId?: string} | null) => {
    if (map && hovered) {
      const featureId: string | undefined = hovered.featureId || getHoveredFeatureId(map, hovered.id)

      if (!hovered.featureId && isCadastreDisplayed) { // Handle parcelle from side menu
        map.setFeatureState({source: CADASTRE_SOURCE, sourceLayer: CADASTRE_SOURCE_LAYER.PARCELLES, id: featureId}, {hover: true})
      ...
      if (prevHoveredParcelle?.current && isCadastreDisplayed) {
        map.setFeatureState({source: CADASTRE_SOURCE, sourceLayer: CADASTRE_SOURCE_LAYER.PARCELLES, id: prevHoveredParcelle.current}, {hover: false})
  ...
  const handleParcelle = useCallback((parcelle: string) => {
  ...
  const toggleCadastreVisibility = useCallback(() => {
    Object.values(CADASTRE_LAYER).forEach((layerId: string) => {
      map.setLayoutProperty(layerId, 'visibility', isCadastreDisplayed ? 'visible' : 'none')
    })
  }, [map, isCadastreDisplayed])

  const filterSelectedParcelles = useCallback(() => {
    if (parcelles.length > 0) {
      const exps: ExpressionSpecification[] = parcelles.map((id: string) => ['==', ['get', 'id'], id])
      map.setFilter(CADASTRE_LAYER.PARCELLES_SELECTED, ['any', ...exps])
    } else {
      map.setFilter(CADASTRE_LAYER.PARCELLES_SELECTED, ['==', ['get', 'id'], ''])
    }
  }, [map, parcelles])

  const filterHighlightedParcelles = useCallback(() => {
    if (parcelles.length > 0) {
      const exps: ExpressionSpecification[] = selectedParcelles.map(id => ['==', ['get', 'id'], id])
      map.setFilter(CADASTRE_LAYER.PARCELLE_HIGHLIGHTED, ['any', ...exps])
    } else {
      map.setFilter(CADASTRE_LAYER.PARCELLE_HIGHLIGHTED, ['==', ['get', 'id'], ''])
    }
  ...
  const reloadParcellesLayers = useCallback(() => {
    ...
    map.setFilter(CADASTRE_LAYER.PARCELLES, ['match', ['get', 'commune'], baseLocale.commune, true, false])
    map.setFilter(CADASTRE_LAYER.PARCELLES_FILL, ['match', ['get', 'commune'], baseLocale.commune, true, false])
  ...
  // Toggle all cadastre layers visiblity
  useEffect(() => {
    if (map && map.getSource(CADASTRE_SOURCE) && isStyleLoaded && isCadastreDisplayed) {
      toggleCadastreVisibility()
    }
