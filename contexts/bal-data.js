...
export const BalDataContextProvider = React.memo(
  ({balId, codeCommune, idVoie, idToponyme, ...props}) => {
    ...
    const reloadNumerosToponyme = useCallback(
      async _idEdited => {
    ...
    const reloadBaseLocale = useCallback(async () => {
      if (balId) {
        ...
        baseLocale.published = Boolean(
          publishedBasesLocales.find(bal => bal._id === baseLocale._id)
        )
