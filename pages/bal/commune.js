...
const Commune = React.memo(({commune, defaultVoies}) => {
  ...
  const onAdd = useCallback(
    async ({nom, positions, typeNumerotation, trace, parcelles}) => {
      if (selectedTab === 'voie') {
        const voie = await addVoie(
          baseLocale._id,
          commune.code,
          {
            nom,
        ...
        const toponyme = await addToponyme(
          baseLocale._id,
          commune.code,
          {
            nom,
  ...
  const onEdit = useCallback(
    async ({nom, typeNumerotation, trace, positions, parcelles}) => {
      if (selectedTab === 'voie') {
        await editVoie(
          editingId,
          {
  ...
  return (
    <>
        ...
        content={
          <Paragraph>
            {`Êtes vous bien sûr de vouloir supprimer ${
              selectedTab === 'voie' ? 'cette voie ainsi que tous ses numéros' : 'ce toponyme'
            } ?`}
          </Paragraph>
        }
      ...
      >
        <div
          className={`tab ${selectedTab === 'voie' ? 'selected' : ''}`}
          onClick={() => setSelectedTab('voie')}
        >
          <Heading>Liste des voies</Heading>
        </div>
        <div
          className={`tab ${selectedTab === 'toponyme' ? 'selected' : ''}`}
          onClick={() => setSelectedTab('toponyme')}
        >
      ...
      {token && voies && voies.length === 0 && (
        <Pane borderTop marginTop='auto' padding={16}>
          <Paragraph size={300} color='muted'>
            Vous souhaitez importer les voies de la commune de {commune.nom} depuis la Base Adresse
            Nationale ?
          </Paragraph>
          <Button
            marginTop={10}
